...
export const FavBarUI = obs(function FavBarUI_(p: { direction?: 'row' | 'column' }) {
   const conf = cushy.preferences.interface.zValue.favBar
   // 💬 2024-09-29 rvion:
   ...
   if (!conf.visible)
      return (
         <Button //
            ...
            onClick={() => (conf.visible = true)}
            children={<Ikon.mdiMenu />}
         />
      )
   ...
   return (
      <Frame //
         ...
         tw='flex flex-col overflow-auto px-1'
      >
         <Button //
            look='ghost'
            base={0}
            tw='flex overflow-auto'
            onClick={(ev) => (conf.visible = false)}
            children={<Ikon.mdiMenu />}
         />
         <Frame //
            base={{ contrast: -0.1 }}
         ...
         >
            <Button
               //
               tw='my-0.5 flex flex-shrink-0 items-center justify-center self-center overflow-clip'
               tooltip='SD1.5'
