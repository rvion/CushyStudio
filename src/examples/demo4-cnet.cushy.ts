WORKFLOW('abccxx', async ({ graph, flow }) => {
    ...
    const upload = await flow.uploadWorkspaceFile(flow.resolveRelative(`assets/test-0.png`))
    // @ts-ignore
    ...
    flow.print(`🟢`)
    flow.print(`🟢 ${JSON.stringify(r1.images[0])}`)
    flow.print(`🟢 ${JSON.stringify(r1.images[0].data.filename)}`)

    for (const i of [1, 2]) {
        // use previous output as base
        const nextBase = graph.WASImageLoad({ image_path: r1.images[0].data.filename })
        const preview = graph.PreviewImage({ images: nextBase })
