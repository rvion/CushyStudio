...
if (platform === 'win32') {
    if (mode === 'dev') executeShellScript('./windows-start-dev.bat')
    else executeShellScript('./windows-start.bat')
} else if (platform === 'linux') {
    // Other OS (like Linux or macOS) - execute .sh script
    if (mode === 'dev') executeShellScript('./maclinux-start-dev.sh')
    else executeShellScript('./maclinux-start.sh')
