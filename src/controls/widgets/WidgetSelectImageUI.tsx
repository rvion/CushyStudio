...
export const WidgetSelectImageUI = observer(function WidgetSelectImageUI_(p: { widget: Widget_image | Widget_imageOpt }) {
    ...
    return (
        <div
            style={dropStyle}
            ref={dropRef}
            className='_WidgetSelectImageUI flex gap-2 p-1 bg-base-100 border border-dashed border-neutral self-center '
        >
            {widget.state.cushy != null ? ( //
                <div tw='flex items-start'>
                    <ImageUI
                        tw='virtualBorder'
                        size={'5rem'}
                        img={draft.db.media_images.getOrThrow(widget.state.cushy.imageID)}
                    />
