...
export function parseDateTimeFR(s: string): Date {
   ...
   if (parsed == null) throw new Error('Invalid date')
...
function parseDateFR(s: string): ParsedDate {
   ...
   if (
      ...
      isNaN(month) ||
      isNaN(day) ||
      (hour != null && isNaN(hour)) ||
      (minute != null && isNaN(minute))
   )
      throw new Error('Invalid date')

   if (day > 31 || month > 12 || (hour != null && hour > 23) || (minute != null && minute > 59))
      throw new Error('Invalid date')
...
export function parseDateTimeEN_US(s: string): Date {
   ...
   if (!Boolean(parsed)) throw new Error('Invalid date')
