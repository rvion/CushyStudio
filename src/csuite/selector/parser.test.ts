...
describe('SelectorParser Tests', () => {
    ...
    it("should parse a selector with all types of filters connected by '|'", () => {
        const parsed = new FieldSelector(".{foo|bar@string|?(@.value === '33')|?(@.type === 'FieldGroup')}").parse()
        const expected: ParsedSelector = {
            steps: [
                { type: 'axis', axis: '.' },
                {
    ...
    it('should throw an error for invalid type filter format', () => {
        expect(() => new FieldSelector('.foo@').parse()).toThrowError(/Expected word at position 5 in selector ".foo@"/)
    })
