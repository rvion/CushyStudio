   ...
   forwardRef(function WidgetStringUI_(p: InputStringProps, ref: ForwardedRef<HTMLInputElement>) {
      ...
      switch (p.type) {
         case 'color':
            ...
            visualHelper = (
               <Frame //
                  tw='UI-Color flex h-full w-full items-center whitespace-nowrap pl-2 font-mono text-[0.6rem]'
      ...
      const input = (
         <input
            ...
            onKeyDown={(ev) => {
               if (ev.key === 'Enter') {
                  // 🔴 Prevents users from submitting the form when pressing enter
      ...
      return (
         <Frame
            ...
            tw={[
               //
               p.icon ? 'pr-1' : 'px-1',
               'UI-InputString h-input relative flex items-center rounded-sm text-sm',
            ]}
            onMouseDown={(ev) => {
               if (ev.button == 1) {
         ...
         >
            ...
            {p.icon != null && (
               <IkonOf //
                  tw='mx-1 flex-none'
                  size='1.2rem'
                  name={p.icon != null ? p.icon : IKONS.mdiText}
               />
            )}
            {p.slotBeforeInput}
            {autoResize ? (
               <div className='minh-input relative'>
                  ...
                  <span
                     ...
                     tw='minh-input lh-input select-none whitespace-nowrap'
                  >
