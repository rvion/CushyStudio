...
export class FieldSelector {
    ...
    selectFrom(from: Field | Field[]): { fields: Field[]; values: any[] } {
        ...
        for (const step of steps) {
            ...
            if (step.type === 'mount') candidates = candidates.filter((node) => node.mountKey === step.key)
                ...
                candidates = candidates.filter((node) => node.type === (step.fieldType === 'str' ? 'str' : step.fieldType))
    ...
    private applyBranch(candidates: Field[], step: StepBranches): Field[] {
        ...
        for (const branch of step.branches) {
            const branchSelector = new FieldSelector('', this.from)
    ...
    parseStep(): ASTStep {
        ...
        else this.FAIL(`Unexpected character '${char}' at position ${this.position} in selector "${this.selector}"`)
    }
    ...
    private consumeNextWord(): string {
        ...
        if (word.length === 0) this.FAIL(`Expected word at position ${this.position} in selector "${this.selector}"`)
        return word
    }
