 ...
 */
export function document_addEventListener<K extends keyof DocumentEventMap>(
   type: K,
   listener: (this: Document, ev: DocumentEventMap[K]) => any,
   options?: boolean | AddEventListenerOptions,
): void

export function document_addEventListener(
   type: string,
...
export function document_addEventListener<K extends keyof DocumentEventMap>(
   ...
   if (!isAction(listener)) throw new Error(`[❌] document_addEventListenerAction must be an action`)
   // 💬 2024-10-07 rvion:
   // | I first tried to use the following code, but it didn't work because
   // | the document.removeEventListener method must be called with the exact same
   // | reference that was passed to document.addEventListener.
