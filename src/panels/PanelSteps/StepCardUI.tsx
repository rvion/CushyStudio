...
export const StepCardUI = obs(function StepOutputsV1HeaderUI_(p: {
   ...
   const appSize = conf.ϟvalue.appSize ? `${conf.ϟvalue.appSize}rem` : '2rem'
   // const outputSize = conf.value.outputSize ? `${conf.value.outputSize}rem` : '2rem'

   const showTitle = p.showTitle ?? conf.ϟvalue.show.title
   const showApp = p.showApp ?? conf.ϟvalue.show.app
   const showDraft = p.showDraft ?? conf.ϟvalue.show.draft
   const showStatus = p.showStatus ?? conf.ϟvalue.show.status
   const showOutputs = p.showOutputs ?? conf.ϟvalue.show.outputs
   const showExecutionTime = p.showExecutionTime ?? conf.ϟvalue.show.executionTime
   const showDate = p.showDate ?? conf.ϟvalue.show.date
