...
await microbench('took', async () => {
   const remove = (distRelPath: string): void => {
      ...
      if (!exists)
         return console.log(`- ${chalk.gray.underline(`./${DIST_RELPATH}/${distRelPath}`)} does not exist`)
   ...
   for (const dep of v.imports) {
      if (dep.external) {
         ...
         if (dep.path.startsWith('./') || dep.path.startsWith('..')) {
            ...
            throw new Error(
               '🔴 some fuckery is happening with external deps detection; probably some import type missing',
            )
         }
...
for (const u of unwantedSorted) {
   ...
   if (!fileInSrcWithExt_ts_exists && !fileInSrcWithExt_tsx_exists) {
      console.log(
         `🔴 ERROR: neither ${fileInSrcWithExt_ts} nor ${fileInSrcWithExt_tsx} exist; skipping COPY.`,
      )
   }
...
if (visited !== jsModuleInBundle.size) {
   ...
   console.error(
      `🔴 WARNING: some files are not visited (allowed: ${jsModuleInBundle.size}, visited: ${visited})`,
   )
} else {
