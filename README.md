...
🖥️ **Cushy Studio**: A modern and cross-platform software to **Play** and **Work** with Generative AI Art. Explore and build generative **🌠 Image**, **🎥 Video**, and **🧊 3d** apps.
...
_screenshot 1: CushyStudio_
![](docs/static/img/screenshots/2023-11-20-00-20-41.png)

- `Left panel`: the general purpose `Cushy Diffusion UI` app. We can see the app controls and a run button at the top
- `Center panel`: The app last output. the output panel is packed with features.
- `Right panel`: the **Library** panel, listing all the built-in apps, and the apps you installed from the **Cushy Library**

---

_screenshot 2: CushyStudio gallery_
...
- Apps with the `ComfyUI` label are imported ComfyUI workflow with an autogenerated interface added on top.
