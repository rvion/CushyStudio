...
export class FormBuilder {
    ...
    group       = <const T extends WidgetDict>(config: Widget_group_config<T>={})                                 => new Unmounted<Widget_group<T>                      >('group', config)
    ...
    stringOpt   = (config: Widget_string_config                                & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_string>  >(config, this.string)
    intOpt      = (config: Omit<Widget_number_config, 'mode'>                  & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_number>  >(config, this.number)
    floatOpt    = (config: Omit<Widget_number_config, 'mode'>                  & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_number>  >(config, this.number)
    numberOpt   = (config: Omit<Widget_number_config, 'mode'>                  & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_number>  >(config, this.number)
    imageOpt    = (config: Widget_image_config                                 & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_image>   >(config, this.image)
    promptOpt   = (config: Widget_prompt_config                                & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_prompt>  >(config, this.prompt)
    colorOpt    = (config: Widget_color_config                                 & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_color>   >(config, this.color)
    groupOpt    = <const T extends WidgetDict>(config: Widget_group_config<T>  & { startActive?: boolean } = {}) => this.wrapOptional<Unmounted<Widget_group<T>>>(config, this.group)
    ...
    private wrapOptional<T extends Unmounted>(
        config: {
