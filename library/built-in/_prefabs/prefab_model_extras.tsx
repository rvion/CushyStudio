...
export type $prefab_model_extras = X.XChoices<{
...
export const prefab_model_extras = (): $prefab_model_extras => {
    ...
    return b.choices(
        {
            ...
            vae: b.enum.Enum_VAELoader_vae_name(),
...
export const run_model_modifiers = (
    extra: $prefab_model_extras['$Value'],
    ckpt: _MODEL,
    forHiRes?: boolean,
    kohyaScale?: number,
): _MODEL => {
    ...
    if (extra.sag && ((!forHiRes && extra.sag.include.base) || (forHiRes && extra.sag.include.hiRes))) {
        ckpt = graph.SelfAttentionGuidance({ scale: extra.sag.scale, blur_sigma: extra.sag.blur_sigma, model: ckpt })
    }

    // 6. Optional PAG - Perturbed Attention Guidance
    if (extra.pag && ((!forHiRes && extra.pag.include.base) || (forHiRes && extra.pag.include.hiRes))) {
        ckpt = graph.PerturbedAttention({ scale: extra.pag.scale, model: ckpt, adaptive_scale: extra.pag.adaptiveScale })
    }
