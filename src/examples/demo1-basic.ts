export default WORKFLOW(async ($) => {
    ...
    const positive = $.CLIPTextEncode({ text: 'masterpiece, (chair:1.3)', clip: ckpt })
    const negative = $.CLIPTextEncode({ text: '', clip: ckpt })
    const sampler = $.KSampler({ seed: $.randomSeed(), steps: 20, cfg: 10, sampler_name: 'euler', scheduler: 'normal', denoise: 0.8, model: ckpt, positive, negative, latent_image: latent, })
