...
export async function _drawCard(
    ...
    const suitLogoImage = await (() => {
        if (suit === 'diamonds') return canvas.createHTMLImage_fromPath('library/built-in/_assets/symbol-diamond.png')
        if (suit === 'clubs') return canvas.createHTMLImage_fromPath('library/built-in/_assets/symbol-club.png')
        if (suit === 'hearts') return canvas.createHTMLImage_fromPath('library/built-in/_assets/symbol-heart.png')
        if (suit === 'spades') return canvas.createHTMLImage_fromPath('library/built-in/_assets/symbol-spades.png')
        return exhaust(suit)
    })()
