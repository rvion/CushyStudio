action({
    name: 'Murphylanga-x-SDXL_face_grap_and_detail.png',
    author: 'murphy',
    ui: (ui) => ({
    ...
    run: async (flow, p) => {
        ...
        const ttN_pipeKSamplerSDXL = graph.ttN_pipeKSamplerSDXL({
            ...
            seed: p['ttN pipeKSamplerSDXL_seed'] ?? flow.randomSeed(),
        ...
        const globalSeed_$3$3Inspire = graph.GlobalSeed_$3$3Inspire({
            ...
            // last_seed: p['GlobalSeed //Inspire_last_seed'] ?? flow.randomSeed(),
        })
        await flow.PROMPT()
    },
})
