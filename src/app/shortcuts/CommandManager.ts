...
export class CommandManager {
    ...
    processKeyDownEvent = (ev: KeyboardEvent<HTMLElement>): RET => {
        ...
        //     if (this.conf.log || s.action == null) this.log(comboInputSequence, `triggered (${s.label})`)
    ...
    tryToRun = (
        ...
        if (this.conf.log) {
            if (res === RET.SUCCESS) this.log('          -> executed')
            // if (res === RET.Failed) this.log('          -> FAILED')
        }

        // continue
        if (res === RET.UNMATCHED) return false
        if (res === RET.FAILED) return false
        // if (res === RET.Failed) continue

        // console.log(s)
        if (res === RET.SUCCESS && s.continueAfterSuccess) return false
