...
describe('auto-migration', () => {
   it('can recover from/to lists', () => {
      ...
      const E1 = S1.create()
      expectJSON(E1.zValue).toBe('🔵')

      // set E1 value to '🟢'
      E1.zValue = '🟢'
      expectJSON(E1.zValue).toBe('🟢')
      expectJSON(E1.zSerial).toMatchObject({ $: 'str', value: '🟢' })

      // construct E2 from E1 serial, but with schema wrapped into list
      const S2 = b.string().list()
      // @ts-expect-error
      const E2 = S2.create(E1.zSerial)

      // E2 should able to PRESERVE the '🟢' when schema has been wrapped into list
      expectJSON(E2.zValue).toMatchObject(['🟢'])
      expectJSON(E2.zSerial).toMatchObject({
         $: 'list',
         items_: [{ $: 'str', value: '🟢' }],
         keys: [E2.items[0]?.zMountKey],
      })
      ...
      // @ts-expect-error
      const E3 = S1.create(E2.zSerial)

      // E3 is able to PRESERVE the '🟢' when schema has been stripped from list
      expectJSON(E3.zSerial).toMatchObject({ $: 'str', value: '🟢' })
   })
   ...
   it('can recover from/to lists + TEST PROXY SETTERS 🔥', () => {
      ...
      expectJSON(E2.zSerial).toMatchObject({
         $: 'list',
         items_: [{ $: 'str', value: '🔴' }],
         keys: [E2.items[0]?.zMountKey as Field_list_ItemID],
      })
