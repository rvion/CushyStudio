...
describe('kolor', () => {
   ...
   it('can properly detect if valid for current screen', () => {
        ...
        expect(c1.isInRec2020Gamut).toBeTruthy() // ✔️
        expect(c1.isInP3Gamut     ).toBeTruthy() // ✔️
        expect(c1.isInRBGGamut    ).toBeTruthy() // ✔️

        // https://oklch.com/#67.35,0.26,0,100
        const c2 = new Kolor(0.6737, 0.26, 0)
        expect(c2.isInRec2020Gamut).toBeTruthy() // ✔️
        expect(c2.isInP3Gamut     ).toBeTruthy() // ✔️
        expect(c2.isInRBGGamut    ).toBeFalsy() // ❌

        // https://oklch.com/#70.29,0.323,0,100
        const c3 = new Kolor(0.7029, 0.323, 0)
        expect(c3.isInRec2020Gamut).toBeTruthy() // ✔️
        expect(c3.isInP3Gamut     ).toBeFalsy() // ❌
        expect(c3.isInRBGGamut    ).toBeFalsy() // ❌

        // https://oklch.com/#80,0.345,0,100
        const c4 = new Kolor(0.8, 0.345, 0)
        expect(c4.isInRec2020Gamut).toBeFalsy() // ❌
        expect(c4.isInP3Gamut     ).toBeFalsy() // ❌
        expect(c4.isInRBGGamut    ).toBeFalsy() // ❌
    })
