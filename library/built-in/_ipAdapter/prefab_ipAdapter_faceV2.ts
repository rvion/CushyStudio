...
export type UI_IPAdapterFaceIDV2 = X.XGroup<{
   ...
   settings: X.XGroup<{
      ...
      models: X.XGroup<{
         type: X.XEnum<'Custom.IPAdapter_plus.IPAdapterUnifiedLoaderFaceID.preset'>
      }>
      ...
      advancedSettings: X.XGroup<{
         ...
         embedding_combination: X.XEnum<'Custom.Impact_Pack.ImpactIPAdapterApplySEGS.combine_embeds'>
               ...
               settings: form.fields(
                  {
                     ...
                     models: form.fields(
                        {
                           type: form.enum['Custom.IPAdapter_plus.IPAdapterUnifiedLoaderFaceID.preset']({
                     ...
                     advancedSettings: form.fields(
                        {
                           ...
                           embedding_combination: form.enum[
                              'Custom.IPAdapter_plus.IPAdapterAdvanced.combine_embeds'
                           ]({
                              default: 'average',
                           }),
                           weight_type: form.enum['Custom.IPAdapter_plus.IPAdapterAdvanced.weight_type']({
                              default: 'linear',
                           }),

                           embedding_scaling: form.enum[
                              'Custom.IPAdapter_plus.IPAdapterAdvanced.embeds_scaling'
                           ]({
                              default: 'V only',
                           }),
...
export type UI_FaceIDImageInput = X.XGroup<{
   ...
   advanced: X.XGroup<{
      ...
      crop_position: X.XEnum<'Custom.IPAdapter_plus.PrepImageForClipVision.crop_position'>
   }>
}>
         ...
         advanced: form.fields(
            {
               ...
               crop_position: form.enum['Custom.IPAdapter_plus.PrepImageForClipVision.crop_position']({
                  default: 'top',
               }),
            },
            {
...
export type UI_extraIpAdapter = X.XGroup<{
   weight: X.XNumber
   embedding_combination: X.XEnum<'Custom.IPAdapter_plus.IPAdapterAdvanced.combine_embeds'>
   ...
   return form.fields(
      {
         ...
         embedding_combination: form.enum['Custom.IPAdapter_plus.IPAdapterAdvanced.combine_embeds']({ default: 'average' }), // prettier-ignore
...
export const run_FaceIDV2 = async (
   ...
   const ip_adapter_loader = graph['Custom.IPAdapter_plus.IPAdapterUnifiedLoaderFaceID']({
      model: ckpt,
   ...
   image = graph['Custom.IPAdapter_plus.PrepImageForClipVision']({
      image,
   ...
   if (ui.settings.advancedSettings.adapterAttentionMask) {
      ...
      const maskClipped = graph['Custom.IPAdapter_plus.PrepImageForClipVision']({
         image: maskLoad,
         crop_position: 'center',
         sharpening: 0,
         interpolation: 'LANCZOS',
      })
      adapterAttentionMask = graph['Extra.ImageToMask']({ image: maskClipped._IMAGE, channel: 'red' })
   }

   for (const ex of ui.settings.extra) {
      const extra = await run.loadImageAnswer(ex.image)
      const image2 = graph['Custom.IPAdapter_plus.PrepImageForClipVision']({
         image: extra._IMAGE,
         crop_position: 'center',
         sharpening: 0,
         interpolation: 'LANCZOS',
   ...
   const faceID = graph['Custom.IPAdapter_plus.IPAdapterFaceID']({
      model: ckpt_pos,
   ...
   if (ui.settings.advancedSettings.extraIPAdapter) {
      const extraIP = graph['Custom.IPAdapter_plus.IPAdapterAdvanced']({
