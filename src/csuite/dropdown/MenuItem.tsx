         ...
         <div tw='flex items-center'>
            ...
            {localShortcut ? (
               <div tw='ml-auto pl-2 text-xs italic'>
                  {localShortcut != null && <ComboUI combo={localShortcut} />}
               </div>
            ) : null}
            {globalShortcut ? (
               <div tw='ml-auto pl-2 text-xs italic'>
                  {globalShortcut != null && <ComboUI combo={globalShortcut} />}
               </div>
            ) : null}
            {afterShortcut}
         </div>
...
export const MenuDivider = observer(function Divider_(p: { children?: ReactNode }) {
   return <div className='!h-input divider my-2 px-2 text-sm'>{p.children ?? <></>}</div>
})
