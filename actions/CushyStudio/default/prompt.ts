...
action({
    ...
    run: async (flow, p) => {
        ...
        const x = _.build_prompt(flow, p.positive, clipAndModelPositive)
        clipAndModelPositive = x.clipAndModel
        const positiveText = x.text

        const y = _.build_prompt(flow, p.negative, clipAndModelPositive)
        ...
        let { latent: LATENT } = await _.build_latent({ flow, opts: p.latent, vae })
