...
export class FormBuilder {
    ...
    // autoUI          =                                                  (p: Widget_str_opts                  ) => new Widget_str                   (this, this.schema, p)
    string             =                                                  (p: W.Widget_str_opts                ) => new W.Widget_str                 (this, this.schema, p)
    color              =                                                  (p: W.Widget_color_opts              ) => new W.Widget_color               (this, this.schema, p)
    size               =                                                  (p: W.Widget_size_opts               ) => new W.Widget_size                (this, this.schema, p)
    stringOpt          =                                                  (p: W.Widget_strOpt_opts             ) => new W.Widget_strOpt              (this, this.schema, p)
    str                =                                                  (p: W.Widget_str_opts                ) => new W.Widget_str                 (this, this.schema, p)
    strOpt             =                                                  (p: W.Widget_strOpt_opts             ) => new W.Widget_strOpt              (this, this.schema, p)
    prompt             =                                                  (p: W.Widget_prompt_opts             ) => new W.Widget_prompt              (this, this.schema, p)
    promptOpt          =                                                  (p: W.Widget_promptOpt_opts          ) => new W.Widget_promptOpt           (this, this.schema, p)
    seed               =                                                  (p: W.Widget_seed_opts               ) => new W.Widget_seed                (this, this.schema, p)
    int                =                                                  (p: W.Widget_int_opts                ) => new W.Widget_int                 (this, this.schema, p)
    intOpt             =                                                  (p: W.Widget_intOpt_opts             ) => new W.Widget_intOpt              (this, this.schema, p)
    float              =                                                  (p: W.Widget_float_opts              ) => new W.Widget_float               (this, this.schema, p)
    floatOpt           =                                                  (p: W.Widget_floatOpt_opts           ) => new W.Widget_floatOpt            (this, this.schema, p)
    number             =                                                  (p: W.Widget_float_opts              ) => new W.Widget_float               (this, this.schema, p)
    numberOpt          =                                                  (p: W.Widget_floatOpt_opts           ) => new W.Widget_floatOpt            (this, this.schema, p)
    matrix             =                                                  (p: W.Widget_matrix_opts             ) => new W.Widget_matrix              (this, this.schema, p)
    boolean            =                                                  (p: W.Widget_bool_opts               ) => new W.Widget_bool                (this, this.schema, p)
    bool               =                                                  (p: W.Widget_bool_opts               ) => new W.Widget_bool                (this, this.schema, p)
    loras              =                                                  (p: W.Widget_loras_opts              ) => new W.Widget_loras               (this, this.schema, p)
    image              =                                                  (p: W.Widget_image_opts              ) => new W.Widget_image               (this, this.schema, p)
    markdown           =                                                  (p: W.Widget_markdown_opts           ) => new W.Widget_markdown            (this, this.schema, p)
    imageOpt           =                                                  (p: W.Widget_imageOpt_opts           ) => new W.Widget_imageOpt            (this, this.schema, p)
    ...
    timeline           = <const T extends W.Widget>                       (p: W.Widget_listExt_opts<T>         ) => new W.Widget_listExt             (this, this.schema, { mode: 'timeline', ...p })
    regional           = <const T extends W.Widget>                       (p: W.Widget_listExt_opts<T>         ) => new W.Widget_listExt             (this, this.schema, { mode: 'regional', ...p })
    ...
    selectOne          = <const T extends { id: string, label?: string}>  (p: W.Widget_selectOne_opts<T>       ) => new W.Widget_selectOne           (this, this.schema, p)
    selectMany         = <const T extends { type: string}>                (p: W.Widget_selectMany_opts<T>      ) => new W.Widget_selectMany          (this, this.schema, p)
    choice             = <const T extends { [key: string]: W.Widget }>    (p: W.Widget_choice_opts<T>          ) => new W.Widget_choice              (this, this.schema, p)
    choices            = <const T extends { [key: string]: W.Widget }>    (p: W.Widget_choices_opts<T>         ) => new W.Widget_choices             (this, this.schema, p)
