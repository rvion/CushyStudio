...
export function codegenSDK(
   ...
   for (const e of this.knownUnionByHash.values()) {
      ...
      for (const alias of e.enumNames) {
         const enumKey = alias
         // p(`    ${escapeJSKey(enumKey)}: { $Name: ${JSON.stringify(enumKey)},'z$Value': Union.${e.unionNameInCushy} },`) // prettier-ignore
         p(`   ${escapeJSKey(enumKey)}: Union['${e.unionNameInCushy}']`) // prettier-ignore
      }
   }
   // for (const n of requirables)
   //    p(`    ${escapeJSKey(n.name)}: { $Name: ${JSON.stringify(n.name)},'z$Value': Union.${n.enum.enumNameInCushy} },`) // prettier-ignore
   p(`}`)
