...
import { document_addEventListener } from '../../csuite/utils/document_addEventListenerAction'
...
export const DrawNodeGraphUI = obs(function DrawNodeGraphUI_(p: {
   //
   spline?: number
...
export const NodeUI = obs(function NodeUI_(p: {
   workflow: ComfyWorkflowL
   node: ComfyNode<any, {}>
   viewRect: ViewRect
}) {
         ...
         {node.visible && node.$schema.pythonModule != 'nodes' && (
            <Frame
               ...
               style={{
                  ...
                  height: bang(node.height) + cushy.preferences.interface.zValue.widgetHeight * 10,
               }}
            >
            ...
            <Frame
               ...
               onMouseDown={(ev) => {
                  if (ev.button == 0) {
                     ...
                     node.selected = true
                     const moveNode = action((ev: MouseEvent): void => {
                        wflow.nodes.map((node) => {
                     ...
                     const stopMoveNode = action((ev: MouseEvent): void => {
                        document.removeEventListener('mousemove', moveNode, false)
                        document.removeEventListener('mouseup', stopMoveNode, false)
                     })
                     document_addEventListener('mousemove', moveNode, false)
                     document_addEventListener('mouseup', stopMoveNode, false)
                     dragging = true
                  }
               }}
               style={{
