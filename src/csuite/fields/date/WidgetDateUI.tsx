...
// date HEADER
export const WidgetDate_HeaderUI = observer(function WidgetDateUI_<VALUE>(p: { field: Field_date<VALUE>; readonly?: boolean }) {
    const field = p.field
    ...
    return (
        <div tw='sticky top-0 flex w-full items-center gap-0.5'>
            <InputStringUI
                ...
                getValue={() =>
                    !field.isEnabledWithinParent || field.value_unchecked == null ? '' : field.format(field.value_unchecked)
                }
                setValue={(value) => {
                    if (p.field.canBeToggledWithinParent) {
                        if (value == '') {
